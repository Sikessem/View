...
use Sikessem\View\Contracts\IsComponentConfig;
use Sikessem\View\Contracts\IsComponentTag;
use Sikessem\View\Contracts\IsManager;

class Manager implements IsManager
{
    final public const COMPONENT_NAMESPACE = 'Sikessem\\View\\Components';

    final public const ANONYMOUS_COMPONENT_NAMESPACE = 'view::components';
    ...
    {
        return config("sikessem.view.$key", $default);
    }
    ...
    {
        return $this->config('prefix', 'view');
    }
    ...
    public function make(string $name, array|ComponentAttributes $attributes = [], string|ComponentSlot|null $slot = null): string
    {
        if ($component = $this->find($name)) {
            ...
            $render = $this->makeComponentTag("x-view-$alias", $attributes, $slot)->toHtml();
